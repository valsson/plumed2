...
the adaptive path method implemented in this action was inspired by the work of Diaz and Ensing in which these formula were used \cite BerndAdaptivePath_Alternative.
...
class AdaptivePath_Alternative : public Mapping {
private:
  OFile pathfile;
  std::string ofmt;
  ...
  TrigonometricPathVessel_Alternative* mypathv;
...
PLUMED_REGISTER_ACTION(AdaptivePath_Alternative,"ADAPTIVE_PATH_ALT")

void AdaptivePath_Alternative::registerKeywords( Keywords& keys ) {
  ...
  normalize_s_values(true)
{
  ...
  std::string input; addVessel("GPATH_ALT", input, -1 );
  readVesselKeywords();
  ...
  mypathv = dynamic_cast<TrigonometricPathVessel_Alternative*>( getPntrToVessel(0) );
...
void AdaptivePath_Alternative::calculate() {
  runAllTasks();
}

void AdaptivePath_Alternative::performTask( const unsigned& task_index, const unsigned& current, MultiValue& myvals ) const {
...
double AdaptivePath_Alternative::transformHD( const double& dist, double& df ) const {
  df=1.0; return dist;
}

void AdaptivePath_Alternative::update() {
